...
- `duration_in_days` (Number) The number of days from the start or end of the month to trigger the alert if the threshold is reached.
...
- `recipient_emails` (List of String) The email addresses that receive the alert, including organization users and verified-domain addresses.
...
- `user_tokens` (List of String) The tokens of organization users that receive the alert. Freeform verified-domain emails are not included; see recipient_emails.
