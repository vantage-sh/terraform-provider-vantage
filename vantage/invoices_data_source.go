	...
	for _, invoice := range result.Payload.Invoices {
		...
		invoiceValue, diag := datasource_invoices.NewInvoicesValue(
			...
			map[string]attr.Value{
				"id":                   types.StringValue(invoice.Token),
				"token":                 types.StringValue(invoice.Token),
				"total":                types.StringPointerValue(invoice.Total),
				"status":               types.StringValue(invoice.Status),
				"billing_period_start": types.StringValue(invoice.BillingPeriodStart),
				"billing_period_end":   types.StringValue(invoice.BillingPeriodEnd),
				"created_at":           types.StringValue(invoice.CreatedAt),
				"updated_at":           types.StringValue(invoice.UpdatedAt),
				"account_name":         types.StringValue(invoice.AccountName),
				"account_token":        types.StringValue(invoice.AccountToken),
				"invoice_number":       types.StringValue(invoice.InvoiceNumber),
				"msp_account_token":    types.StringValue(invoice.MspAccountToken),
			},
		)
		if diag.HasError() {
