...
func (m *managedAccountModel) applyPayload(ctx context.Context, payload *modelsv2.ManagedAccount) diag.Diagnostics {
	m.Name = types.StringValue(payload.Name)
	m.ContactEmail = types.StringValue(payload.ContactEmail)
	m.Id = types.StringValue(payload.Token) // Set ID to the token for resource identification

	// For resource operations, keep the planned value to avoid delegation/mixed credential issues.
	// i.e. The endpoint will attempt to return all child access credentials and not just the parent access credentials that are being delegated.
	if m.AccessCredentialTokens.IsUnknown() {
		m.AccessCredentialTokens = types.ListNull(types.StringType)
	}

	// For resource operations, keep the planned value to avoid order/consistency issues
	if m.BillingRuleTokens.IsUnknown() {
		m.BillingRuleTokens = types.ListNull(types.StringType)
	}
