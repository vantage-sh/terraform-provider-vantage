...
// ---------------------------------------------------------------------------
// Removing optional nested blocks clears state
// ---------------------------------------------------------------------------

// TestAccVantageCostReport_settingsRemovedClearsState verifies that when the
// `settings` block is removed from config (and the API returns nil for it),
// Terraform state is reset to null instead of retaining the stale value.
//
// Regression test for a bug in applyPayload where, if state had a non-null
// settings value and the API payload returned nil, neither the "populate"
// nor the "set to null" branch executed. That left stale data in state and
// subsequent Read calls perpetuated the drift.
func TestAccVantageCostReport_settingsRemovedClearsState(t *testing.T) {
	rTitle := sdkacctest.RandStringFromCharSet(10, sdkacctest.CharSetAlphaNum)
	resourceName := "vantage_cost_report.test"
	...
	resource.Test(t, resource.TestCase{
		...
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			// Step 1: Create with explicit settings block populated.
			{
				Config: testAccCostReportConfig_settings(rTitle, true, true, true),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "title", rTitle),
			...
			// Step 2: Remove the settings block from config. After apply the
			// attribute should be null in state (no stale value retained).
			{
				Config: testAccCostReportConfig_noSettings(rTitle),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckNoResourceAttr(resourceName, "settings.amortize"),
					resource.TestCheckNoResourceAttr(resourceName, "settings.include_credits"),
					resource.TestCheckNoResourceAttr(resourceName, "settings.include_tax"),
				),
			},
			// Step 3: Verify no drift — the subsequent Read should keep state
			// null. Before the fix this step would re-materialize the stale
			// settings object and trigger perpetual drift.
			{
				Config:             testAccCostReportConfig_noSettings(rTitle),
				PlanOnly:           true,
				ExpectNonEmptyPlan: false,
...
// TestAccVantageCostReport_chartSettingsRemovedClearsState is the analogous
// regression test for the chart_settings block.
func TestAccVantageCostReport_chartSettingsRemovedClearsState(t *testing.T) {
	rTitle := sdkacctest.RandStringFromCharSet(10, sdkacctest.CharSetAlphaNum)
	resourceName := "vantage_cost_report.test"
	...
	resource.Test(t, resource.TestCase{
		...
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			// Step 1: Create with chart_settings populated.
			{
				Config: testAccCostReportConfig_chartSettings(rTitle, "date", "cost"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "chart_settings.x_axis_dimension.#", "1"),
			...
			// Step 2: Remove the chart_settings block from config.
			{
				Config: testAccCostReportConfig_noChartSettings(rTitle),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckNoResourceAttr(resourceName, "chart_settings.x_axis_dimension.#"),
					resource.TestCheckNoResourceAttr(resourceName, "chart_settings.y_axis_dimension"),
				),
			},
			// Step 3: Verify no drift.
			{
...
func testAccCostReportConfig_noSettings(title string) string {
	return fmt.Sprintf(`
data "vantage_workspaces" "test" {}
...
func testAccCostReportConfig_noChartSettings(title string) string {
	return fmt.Sprintf(`
data "vantage_workspaces" "test" {}
